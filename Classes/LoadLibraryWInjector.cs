...
{
    public static class LoadLibraryWInjector
    {
        ...
        public static bool Inject(string path, string name)
        {
            try
            {
                ...
                string fullPath = Path.GetFullPath(path); // Ensure full absolute path
                ...
                try
                {
                    ...
                    uint size = (uint)((fullPath.Length + 1) * 2); // UTF-16 wchar_t
                    ...
                    if (remoteModuleHandle == 0)
                        throw new Exception("LoadLibraryW failed in remote process (exit code = 0).");
