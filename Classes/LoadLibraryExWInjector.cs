...
{
    public static class LoadLibraryExWInjector
    {
        ...
        public static bool Inject(string path, string name)
        {
            try
            {
                ...
                try
                {
                    ...
                    IntPtr loadLibraryExAddress = NativeMethods.GetProcAddress(kernel32Handle, "LoadLibraryExW");
                    if (loadLibraryExAddress == IntPtr.Zero)
                        throw new Win32Exception(Marshal.GetLastWin32Error(), "Failed to get LoadLibraryExW address.");

                    uint size = (uint)((path.Length + 1) * 2); // Unicode string size (2 bytes per char)
                    ...
                    byte[] buffer = Encoding.Unicode.GetBytes(path); // Using Unicode encoding
                    ...
                    if (remoteModuleHandle == 0)
                        throw new Exception("LoadLibraryExW failed in remote process (exit code = 0).");
                }
                finally
                {
                    if (threadHandle != IntPtr.Zero)
